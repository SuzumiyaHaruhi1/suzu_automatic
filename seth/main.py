...
class RDPProxy(threading.Thread):
    ...
    def __init__(self, local_conn, remote_socket, addr):
...
def open_sockets(port):
    ...
    return local_conn, remote_socket, addr[0]
...
def run():
    try:
        while True:
            lsock, rsock, addr = open_sockets(args.target_port)
            RDPProxy(lsock, rsock, addr).start()
